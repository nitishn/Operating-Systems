...
// global constants
	// initialize num produced/moved/consumed counts
		int itemsProduced = 0;
		int itemsMoved = 0;
		int itemsConsumed = 0;	
	...
	// initialize mutex and semaphore constants
		...
		sem_t produce, move, consume, limit;

	// initialize buffers
		buffer myBuffer(BUFFER_PROD_SIZE);
	   buffer myMoverBuffer(BUFFER_MOVE_SIZE);
	...
	{	
		// parse command line arguments								
			// exit program if command line arguments are invalid
				if((argc != 5) || (SLEEP_TIME < 0) || (NUM_PRODUCERS < 0) || (NUM_CONSUMERS < 0) || (NUM_MOVERS < 0))
					{
						cout << "\nPlease enter 4 valid arguments after the file name (int int int int)\nProgram Exited\n";				
						return 0;
					}
			...
			// set number of mover threads
				NUM_MOVERS = atoi(argv[4]);
		...
		// initialize unique thread ID for producer/mover/consumer threads
			int proId = 0;
			int conId = 0;												
			int movId = 0;
		...
		// create mover thread(s)
			pthread_t threadMov[NUM_MOVERS];						
			for(int i = 0; i < NUM_MOVERS; i++, movId++)											
				pthread_create(&(threadPro[i]), NULL, mover, &movId);														
		...
		// print out number of producers and consumers			
			...
			cout << "Items moved: " << itemsMoved << endl;							
		...
		while(true)
			{
				...
				// remove item from buffer & report if failed or not		
					if(myBuffer.remove_item(item))
						cout << "BUFFER EMPTY, item not moved" << endl;

				//	move item into mover buffer & report if failed or not			
					if(myMoverBuffer.insert_item(item))
						cout << "BUFFER FULL, item not moved" << endl;

				// display info to screen if move was sucessful
					else					
						{
							// display which thread consumed which item
								cout << "\tMover " << ID << " moved: " << item << endl;
								itemsMoved++;
						}							
		...
		while(true)
			{
				...
				// consume item from buffer & report if failed or not		
					if(myMoverBuffer.remove_item(item))
